...
class HardSigmoid2EmbeddingSteadyStateModel(BaseModel):
    embeddings: jnp.ndarray | None = None
    embedding_dim: int | None = None

    def setup(self):
        # approach 1
        # self.W = self.param("W", glorot_normal(), (self.embedding_dim, self.embedding_dim))
        ...
        self.W = self.param("W", glorot_normal(), (self.n_genes, self.embedding_dim))
        self.bias_term_sigmoid_ = self.param("bias_term_sigmoid_", zeros, (self.n_genes,))
        self.scale_factor_ = self.param("scale_factor_", ones, (self.n_genes,))
    ...
    def get_Amat(self):
        # approach 1
        # G_transformed = jnp.dot(self.embeddings, self.W)
        # Amat = jnp.dot(G_transformed, self.embeddings.T)

        # approach 2
        Amat = jnp.dot(self.W, self.embeddings.T)

        Amat = Amat * (1.0 - jnp.eye(self.n_genes))
