...
def _simulate_ko_chunk(gene_chunk: List[str]):
    ...
    for gene_name in gene_chunk:
        with model:
            try:
                ...
                if gene_name not in model.genes:
                    continue
                
                model.genes.get_by_id(gene_name).knock_out()
    ...
    def fit(self, model: cobra.Model, genes: List[str], **kwargs):
        ...
        if self.cache_file and os.path.exists(self.cache_file):
            ...
            results = Parallel(n_jobs=self.n_jobs)(
                delayed(_simulate_ko_chunk)(chunk) for chunk in chunks
            )
            
            # Consolidate results in memory
            valid_results = [r for r in results if r]
            if valid_results:
                self.fluxes = pd.concat([pd.DataFrame.from_dict(r, orient='index') for r in valid_results])
            else:
